...
# 부서번호, 부서명, 직원번호, 직원명 Cross Join으로 조회하기
SELECT d.dept_id, dept_name, emp_id, e.emp_name
...
# 모든 직원들의 부서번호, 부서명, 직원번호, 직원명을 조회하기
SELECT d.dept_id, dept_name, emp_id, emp_name   # 두 개의 테이블의 중복적으로 들어간 칼럼만 별칭(d, e)를 붙이면 됨
FROM tbl_department d
INNER JOIN tbl_employee e
ON d.dept_id = e.dept_id;   -- 두 테이블의 참조 관계를 찾아서 이를 조인 조건으로 사용합니다.

# EXPLAIN을 쿼리 실행계획 확인하기
EXPLAIN SELECT d.dept_id, dept_name, emp_id, e.emp_name
FROM tbl_department d
INNER JOIN tbl_employee e
...
# 모든 직원들의 부서아아디, 부서명, 직원아이디, 직원명을 조회하기
...
FROM tbl_department d
INNER JOIN tbl_employee e
ON d.dept_id = e.dept_id AND dept_location = "서울";
...
SELECT DISTINCT d.dept_id, dept_name, dept_location           -- DISTINCT: 중복 방지
FROM tbl_department d
INNER JOIN tbl_employee e
ON d.dept_id = e.dept_id AND salary >= 5000000;


# 3. 각 지역별로 근무하는 직원 수 조회하기 (근무 중인 직원이 없으면 조회할 필요 없음)
SELECT dept_location, 
        COUNT(emp_id) -- 직원 수를 구하기 위해서 모든 직원이 값을 가지고 있는 emp_id(PK)를 COUNT 함수에 전달
FROM tbl_department d
INNER JOIN tbl_employee e
ON d.dept_id = e.dept_id
GROUP BY dept_location;   # GROUP BY는 GROUP으로 묶은 항목만 조회 가능
...
LEFT OUTER JOIN tbl_employee e
ON d.dept_id = e.dept_id
GROUP BY d.dept_id, dept_name;  # 이름으로 조회하는건 중복 위험등이 있기 때문에 id도 같이 조회