...
# 직원 아이디가 1001인 직원과 같은 직급의 직원 정보 조회하기
SELECT emp_id, dept_id, emp_name, position, gender, hire_date, salary       
FROM tbl_employee                                                            -- main query
WHERE position = (SELECT position FROM tbl_employee WHERE emp_id = 1001); -- 단일 행 서브쿼리 (단일 행 연산자 : =, !=, >, >=, <, <=), 갈호 안에 있는 게 서브쿼리
...
# 직원 아이디가 1003인 직원 정보와 잔체 직원의 급여 평균을 함께 조회
SELECT emp_id, dept_id, emp_name, position, gender, hire_date, salary,
...
/*
  ...
  1. 서브쿼리가 메인쿼리의 값을 참조하는 방식의 쿼리 입니다.
