/*
  ...
  2. 각 절의 실행 순서
    ...
    7) LIMIT 절
      지금까지 과정을 거친 행(데이터)을 조회할 때 개수나 범위를 제한합니다. oracle에는 없는 문법
  ...
  3. 기타 특징
    ...
    5) LIMIT 절의 시작 값은 생략할 수 있습니다. 이 경우 데이터는 처음부터 가져옵니다. (첫 행의 시작 값은 0입니다.)
*/
...
# 2-3) 부서명과 위치만 조회하되, "부서명", "위치"라는 칼럼 별칭 사용하기 (ㅒ""는 써도 되고 안써도 됨 공백이 있는 경우만 필수)
...
# 2-5) 부서 위치만 조회하되, 중복 제거 후 조회하기 (DISTINCT: 중복 제거)
...
SELECT *
FROM tbl_department
ORDER BY dept_name ASC;  # ASC: 오름차순(가나다순), 생략 가능
...
SELECT *
FROM tbl_department
LIMIT 0, 2;   -- 첫 번째 부서를 의미합, 2: 2개 부서를 의미함 
