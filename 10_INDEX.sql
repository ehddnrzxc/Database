...
# 부서아이디가 1인 부서 조회하기: 인덱스 태우기 성공 (PK의 정확한 위치를 파악하고 결과 냄)
EXPLAIN 
SELECT dept_id, dept_name, dept_location
FROM tbl_department
WHERE dept_id = 1;  -- 인덱스가 설정된 칼럼을 그대로 WHERE절에서 사용합니다.
# 결과
# type=const: PK 또는 UK(UNIQUE KEY)를 WHERE절에서 상수 값과 비교하여 항상(const) 하나의 행만 반환하는 접근 방식
...
# 결과
# type=ALL: 테이블의 모든행을 처음부터 모두 읽는 Full Table scan 방식 (가장 안 좋은 방식)
