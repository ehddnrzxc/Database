...
CREATE TABLE IF NOT EXISTS tbl_department (
  ...
  FOREIGN KEY(manager_id) REFERENCES tbl_employee(emp_id)  # 직원 테이블이 있어야만 부서 테이블을 만들 수 있습니다.
    ON DELETE SET NULL  # 부서장이 없어지면 manager_id를 NULL 처리합니다.
) ENGINE=InnoDB;
...
# 직원 테이블 dept_id 칼럼 추가하고 외래키 제약조건 추가
...
# 직원, 부서 테이블 삭제
DROP TABLE IF EXISTS tbl_department, tbl_employee;
